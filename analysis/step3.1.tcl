...
# @Last modified time: 2019-05-24T11:19:56-04:00
# @Comment:
...
set nf [molinfo $pep get numframes]
puts $outputFile "# resid       psi     omega     phi"
for { set i 0 } { $i < $nf } { incr i } {
     for { set rnum 1 } { $rnum < $nr } { incr rnum } {
          set xyz1 [lindex [[atomselect $pep "protein and resid $rnum and name N" frame $i ] get {x y z}] 0 ]
          set xyz2 [lindex [[atomselect $pep "protein and resid $rnum and name CA" frame $i ] get {x y z}] 0 ]
          set xyz3 [lindex [[atomselect $pep "protein and resid $rnum and name C" frame $i ] get {x y z}] 0 ]
          set xyz4 [lindex [[atomselect $pep "protein and resid [expr {$rnum + 1}] and name N" frame $i ] get {x y z}] 0 ]
          set xyz5 [lindex [[atomselect $pep "protein and resid [expr {$rnum + 1}] and name CA" frame $i ] get {x y z}] 0 ]
          set xyz6 [lindex [[atomselect $pep "protein and resid [expr {$rnum + 1}] and name C" frame $i ] get {x y z}] 0 ]
          set psi [calc $xyz1 $xyz2 $xyz3 $xyz4]
          set omega [calc $xyz2 $xyz3 $xyz4 $xyz5]
          set phi [calc $xyz3 $xyz4 $xyz5 $xyz6]
          puts $outputFile [format "%4s %6s %10.1f %7.1f %7.1f" $i $rnum $psi $omega $phi]
     }
}
close $outputFile
exit
